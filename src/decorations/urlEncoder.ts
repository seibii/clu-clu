...
const mapKeyAndStringValue = (object: ParsedQuery, prefix: string): [string, string][] =>
  Object.keys(object)
    .filter((key) => object[key])
    .map((key): [string, unknown] => [prefix === '' ? key : `${prefix}[${key}]`, object[key]])
    .reduce((encoded: [string, string][], [key, value]): [string, string][] => {
      if (!isChildren(value)) return encoded.concat([[key, stringfyValue(value)]]);
...
const isDate = (value: unknown): value is Date => typeof value === 'object' && value instanceof Date;

const isChildren = (value: unknown): boolean => typeof value === 'object' && !isDate(value);
