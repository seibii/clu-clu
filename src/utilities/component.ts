...
import { AnalyticsRequest, AnalyticsSource } from '../infrastructure/drivers/analyticsDriver';
...
import { FlashRequest, FlashSource } from '../infrastructure/drivers/flashDriver';
import { LocationRequest } from '../infrastructure/drivers/locationDriver';
import { MediaDevicesSource, MediaRequest } from '../infrastructure/drivers/mediaDevicesDriver';
import { MediaQuerySource } from '../infrastructure/drivers/mediaQueryDriver';
import { ShakenQRRequest, ShakenQRSource } from '../infrastructure/drivers/shakenQRDriver';
...
import { ModalActionRequest, ModalActionSource } from '../infrastructure/drivers/modalActionsDriver';
...
import { CryptoRequest, CryptoSource } from '../infrastructure/drivers/cryptoDriver';
...
export const mergeSinksAndOverrideDOM = <BaseState, OverrideState>(
  ...
  DOM: dom,
  flash: override.flash && flash ? Stream.merge(base.flash, override.flash, flash.flash) : base.flash,
  state: (override.state ? Stream.merge(override.state, base.state) : base.state) as Stream<Reducer<BaseState>>
});

export type Intent<Actions, State> = (sources: Sources<State>) => Actions;

export type Model<Actions, State> = (actions: Actions) => optionalModelResult<State>;
...
export type optionalModelResult<State> = { [K in keyof ModelResult<State>]?: ModelResult<State>[K] };

export type View<S> = (state$: Stream<S>) => Stream<VNode>;

export const createComponent = <A, S>(intent: Intent<A, S>, model: Model<A, S>, view: View<S>) => (
  sources: Sources<S>
): Sinks<S> => {
...
export const mergeStreamSinks = <State>(app: Sinks<State>, page$: Stream<Sinks<State>>): Sinks<State> => {
  return Object.keys(app)
    .map((key: keyof Sinks<State>): [keyof Sinks<State>, Stream<unknown>] => [
      key,
      Stream.merge(app[key] as Stream<unknown>, page$.map((component) => component[key] as Stream<unknown>).flatten())
    ])
    .reduce((acc, [key, val]) => Object.assign(acc, { [key]: val }), {}) as Sinks<State>;
};
