...
export type Auth0Request = Auth0TokenRequest | Auth0LoginRequest | Auth0LogoutRequest;
...
export const makeAuth0Driver = (props: Props): ((stream: Stream<Auth0Request>) => Auth0Source) => {
  ...
  return (stream: Stream<Auth0Request>): Auth0Source => {
    ...
    stream
      ...
      .map((request: Auth0LoginRequest) =>
        Stream.fromPromise(client.loginWithRedirect({ connection: request.connection }))
      )
      .flatten()
      .addListener({
    ...
    stream
      .filter((request): request is Auth0LogoutRequest => request.type === 'logout')
      .addListener({
        next: (request) => {
          client.logout({ returnTo: request.returnUri ? `${props.redirectUrl}${request.returnUri}` : props.redirectUrl });
